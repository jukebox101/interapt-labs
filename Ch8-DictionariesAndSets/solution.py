...
# Generate and return a random list of no more than num_items items
# Each integer between 0 and 15
def gen_return_random_unique_num_list( num_items ) :
    return list( { random.randint( 0, 20 ) for idx in range(0, num_items) } )
...
def main( ):
    ...
    buncha_iters = [ gen_return_random_unique_num_list( num_each_iterator )
                                   for iter_idx in range( 0, num_iterators ) ]
